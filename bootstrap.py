...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    print(c("[1/3] VERIFYING SYSTEM CORE …", BOLD + CYAN))
    if cmd_check(args) != 0:
        sys.exit(1)

    # Phase 2 — Parallel install + build
    print(c("[2/3] ASSEMBLING COMPONENTS (parallel) …", BOLD + CYAN))
    ...
    print(c("[3/3] INITIALIZING INTELLIGENCE …", BOLD + CYAN))
    _setup_env_file()
    ...
    if not env_file.exists():
        ...
        with env_file.open("w", encoding="utf-8") as f:
            f.write(
                ...
                f"FLASK_HOST=127.0.0.1\n"
                f"FLASK_DEBUG=True\n"
                f"DATABASE_URL=postgresql://postgres:postgres@localhost:5432/postgres\n"
                f"OPENAI_API_KEY=your_actual_openai_api_key_here\n"
        ...
        print(c(f"  .env created at {env_file}  — fill in your secrets.", YELLOW))
        return
