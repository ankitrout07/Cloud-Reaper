...
class AnomalyBridge(BaseGoBridge):
    ...
    def __init__(
        self, host: str = "localhost", port: int = 7076, timeout: float = 30.0, enabled: bool = True
    ):
        """
    ...
    async def batch_detect_anomalies(self, metrics: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        """
        return await self._make_request(
            "POST", "/api/anomaly/batch_detect", json_data={"metrics": metrics}
        )
    ...
    async def get_metric_buffer(self, metric_name: str) -> dict[str, Any]:
        ...
        """
        return await self._make_request(
            "GET", "/api/anomaly/buffer", params={"metric_name": metric_name}
        )

    async def clear_buffer(self, metric_name: str | None = None) -> dict[str, Any]:
        """
        ...
        """
        return await self._make_request(
            "POST", "/api/anomaly/clear_buffer", json_data={"metric_name": metric_name or ""}
        )

    async def set_config(
        self, threshold: float | None = None, window_size: int | None = None
    ) -> dict[str, Any]:
        """
        ...
        return await self._make_request("POST", "/api/anomaly/config", json_data=config_data)
...
def create_anomaly_bridge(**kwargs) -> AnomalyBridge:
    ...
    """
    return create_bridge_client(AnomalyBridge, "anomaly", **kwargs)


# Global singleton instance
_global_anomaly_bridge: AnomalyBridge | None = None
