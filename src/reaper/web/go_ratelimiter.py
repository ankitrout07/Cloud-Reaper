...
class GoRateLimiter(BaseGoBridge):
    ...
    async def allow(self, key: Optional[str] = None) -> bool:
        """
    ...
    async def wait(self, key: Optional[str] = None) -> float:
        """
    ...
    async def get_statistics(self, key: Optional[str] = None) -> Dict[str, Any]:
        """
...
_global_rate_limiter: Optional[GoRateLimiter] = None
...
async def wait_for_rate_limit(key: Optional[str] = None) -> float:
    """
    Wait until rate limit allows proceeding.
