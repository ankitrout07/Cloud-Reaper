...
from reaper.governance.best_practices import PracticeCategory, ProviderBestPracticesEngine
...
async def list_policies(category: str = None, status: str = None):
    ...
    try:
        ...
        templates = policy_engine.list_templates(category=policy_category, status=policy_status)
...
async def get_policy(policy_id: str):
    ...
    try:
        ...
        return {"status": "success", "policy": policy_engine._template_to_dict(template)}
...
async def evaluate_policy_compliance(request: PolicyComplianceRequest):
    ...
    try:
        result = policy_engine.evaluate_policy_compliance(
            request.policy_id, request.resources, request.provider
        )
...
async def compare_providers(request: ProviderComparisonRequest):
    ...
    try:
        result = migration_advisor.compare_providers(
            request.resource_inventory, request.current_provider
        )
...
async def generate_multi_cloud_report(request: MultiCloudReportRequest):
    ...
    try:
        ...
        result = cost_aggregator.generate_multi_cloud_report(cost_records, period_start, period_end)
...
async def compare_provider_costs(request: ProviderCostComparisonRequest):
    ...
    try:
        ...
        result = cost_aggregator.compare_provider_costs(cost_records, request.period_days)
        return {"status": "success", "comparison": result}
    except Exception as e:
...
async def forecast_costs(request: CostForecastRequest):
    ...
    try:
        ...
        result = cost_aggregator.forecast_costs(cost_records, request.forecast_days)
...
async def list_best_practices(provider: str = None, category: str = None):
    ...
    try:
        ...
        practices = best_practices_engine.list_practices(
            provider=provider, category=practice_category
        )
...
async def get_best_practice(practice_id: str):
    ...
    try:
        ...
        return {"status": "success", "practice": best_practices_engine._practice_to_dict(practice)}
    except HTTPException:
        raise
    except Exception as e:
...
async def evaluate_practice(request: PracticeEvaluationRequest):
    ...
    try:
        result = best_practices_engine.evaluate_practice(request.practice_id, request.resources)
...
async def evaluate_provider_compliance(request: ProviderComplianceRequest):
    ...
    try:
        result = best_practices_engine.evaluate_provider_compliance(
            request.provider, request.resources
        )
        return {"status": "success", "compliance": result}
    except Exception as e:
...
async def generate_recommendations(request: RecommendationsRequest):
    ...
    try:
        result = best_practices_engine.generate_recommendations(
            request.provider, request.resources, request.max_recommendations
        )
...
async def health_check():
    ...
    return {
        ...
        "services": {
            ...
            "migration_advisor": "operational"
            if GOVERNANCE_ENABLED and migration_advisor
            else "disabled",
            "cost_aggregator": "operational"
            if GOVERNANCE_ENABLED and cost_aggregator
            else "disabled",
            "best_practices_engine": "operational"
            if GOVERNANCE_ENABLED and best_practices_engine
            else "disabled",
        },
