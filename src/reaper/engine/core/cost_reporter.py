...
class CostSavingImpact:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "savings_percentage": (self.monthly_savings / self.original_monthly_cost * 100) if self.original_monthly_cost > 0 else 0,
...
class CostTrend:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "savings_percentage": (self.savings / self.total_cost * 100) if self.total_cost > 0 else 0,
...
class AutomatedCostReporter:
    ...
    def generate_executive_summary(self, 
        ...
        implementation_rate = (
            ...
            len(self.saving_impacts) if self.saving_impacts else 0
        ) if self.saving_impacts else 0
        ...
        # Priority breakdown
        priority_breakdown = {
            "critical": len([r for r in self.recommendations_history if r.get("priority") == "critical"]),
            "high": len([r for r in self.recommendations_history if r.get("priority") == "high"]),
            "medium": len([r for r in self.recommendations_history if r.get("priority") == "medium"]),
    ...
    def _get_implementation_progress(self) -> Dict[str, Any]:
        ...
        status_counts = {
            "planned": len([r for r in self.recommendations_history if r.get("status") == "planned"]),
            "in_progress": len([r for r in self.recommendations_history if r.get("status") == "in_progress"]),
            "completed": len([r for r in self.recommendations_history if r.get("status") == "completed"]),
    ...
    def _generate_resource_details(self) -> Dict[str, Any]:
        ...
        for rec in self.recommendations_history:
            ...
            resource_summary[resource_id]["total_potential_savings"] += rec.get("estimated_monthly_savings", 0)
    ...
    def _generate_category_analysis(self) -> Dict[str, Any]:
        ...
        for rec in self.recommendations_history:
            ...
            cat_analysis["implementation_effort_distribution"][rec.get("implementation_effort", "low")] += 1
    ...
    def _generate_risk_assessment(self) -> Dict[str, Any]:
        ...
        sorted_recs = sorted(
            self.recommendations_history,
            key=lambda x: (risk_order.get(x.get("risk_level", "low"), 2), -x.get("estimated_monthly_savings", 0))
        )
    ...
    def _generate_next_steps(self) -> List[str]:
        ...
        critical_count = len([r for r in self.recommendations_history if r.get("priority") == "critical"])
        if critical_count > 0:
            next_steps.append(f"Address {critical_count} critical priority recommendations immediately")
        
        # Check implementation progress
        completed_count = len([i for i in self.saving_impacts if i.implementation_status == "completed"])
        total_count = len(self.saving_impacts)
        
        if total_count > 0 and completed_count < total_count:
            next_steps.append(f"Complete {total_count - completed_count} in-progress implementations")
        
        # Check for safe, high-value opportunities
        quick_wins = len([
            r for r in self.recommendations_history
            if r.get("risk_level") in ["safe", "low"] and 
               r.get("estimated_monthly_savings") > 500 and
               r.get("implementation_effort") == "low"
        ])
        if quick_wins > 0:
            next_steps.append(f"Implement {quick_wins} quick-win opportunities for immediate savings")
    ...
    def _convert_to_markdown(self, report: Dict[str, Any]) -> str:
        ...
        md.append(f"- **Total Potential Monthly Savings:** ${summary['total_potential_savings']:,.2f}")
    ...
    def _convert_to_html(self, report: Dict[str, Any]) -> str:
        ...
        html.append(".summary-card { background: #f0f9ff; padding: 20px; border-radius: 8px; margin: 20px 0; }")
        html.append(".recommendation { border: 1px solid #ddd; padding: 15px; margin: 10px 0; border-radius: 5px; }")
        ...
        html.append(f"<p><strong>Period:</strong> {executive['report_period'].upper()} | <strong>Provider:</strong> {executive['provider'].upper()}</p>")
        html.append(f"<p><strong>Generated:</strong> {executive['generated_at']}</p>")
        
        summary = executive["summary"]
        html.append("<div class='summary-card'>")
        html.append("<h2>Executive Summary</h2>")
        html.append(f"<p><strong>Total Potential Monthly Savings:</strong> ${summary['total_potential_savings']:,.2f}</p>")
        html.append(f"<p><strong>Total Implemented Savings:</strong> ${summary['total_implemented_savings']:,.2f}</p>")
        html.append(f"<p><strong>Savings Realization Rate:</strong> {summary['savings_realization_rate']:.1f}%</p>")
        html.append(f"<p><strong>Total Recommendations:</strong> {summary['total_recommendations']}</p>")
        html.append("</div>")
        ...
        for i, rec in enumerate(executive["top_opportunities"], 1):
            ...
            html.append(f"<p><strong>Savings:</strong> ${rec.get('estimated_monthly_savings', 0):,.2f}/month")
            html.append(f"<p><strong>Priority:</strong> {rec.get('priority', 'unknown').upper()}</p>")
            html.append(f"<p><strong>Risk:</strong> {rec.get('risk_level', 'unknown').title()}</p>")
            html.append(f"<p><strong>Description:</strong> {rec.get('description', 'No description')}</p>")
            html.append("</div>")
    ...
    def _convert_to_csv(self, report: Dict[str, Any]) -> str:
        ...
        csv_lines.append("Recommendation Title,Category,Priority,Risk Level,Monthly Savings,Savings %,Resource Name,Implementation Effort")
    ...
    def add_cost_trend(self, period: str, total_cost: float, 
                      optimized_cost: float, resource_count: int,
                      recommendations_count: int, implemented_count: int):
