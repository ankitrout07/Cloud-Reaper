    ...
    def analyze_compute_waste_index(self, active_inventory: list) -> list:
        ...
        for asset in active_inventory:
            ...
            matching_instance = next(
                (inst for inst in cpu_metrics if instance_ip and inst and instance_ip in inst), None
            )
            avg_utilization = (
                cpu_metrics.get(matching_instance, 50.0) if matching_instance else 50.0
            )
