                ...
                if isinstance(child, ast.Call):
                    if isinstance(child.func, ast.Attribute):
                        if (
                            ...
                            print(
                                f"{filepath}:{child.lineno}: Blocking subprocess.run in async function '{node.name}'"
                            )
                        if (
                            ...
                            print(
                                f"{filepath}:{child.lineno}: Blocking time.sleep in async function '{node.name}'"
                            )
                ...
                if isinstance(child, ast.Call):
                    if isinstance(child.func, ast.Attribute) and child.func.attr in (
                        ...
                        if (
                            ...
                            print(
                                f"{filepath}:{child.lineno}: Blocking requests.{child.func.attr} in async function '{node.name}'"
                            )
        ...
        if isinstance(node, ast.FunctionDef) or isinstance(node, ast.AsyncFunctionDef):
            for default in node.args.defaults + node.args.kw_defaults:
                if isinstance(default, (ast.List, ast.Dict)):
                    print(
                        f"{filepath}:{node.lineno}: Mutable default argument in function '{node.name}'"
                    )
