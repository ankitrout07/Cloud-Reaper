...
content = re.sub(r"time\.sleep\(0\.5\)", r"await asyncio.sleep(0.5)", content)

# 4. Remove redundant 'import asyncio' in try blocks
content = re.sub(r"(\s+)import asyncio\s+await asyncio\.sleep", r"\1await asyncio.sleep", content)
...
content = re.sub(
    ...
    lambda m: m.group(0).replace(
        f"AzureCollector().{m.group(2)}",
        f"await asyncio.to_thread(lambda: AzureCollector().{m.group(2)})",
    ),
    content,
    flags=re.MULTILINE,
)
